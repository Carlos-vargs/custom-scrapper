...
app.get('/scrape/soianet', async (req, res) => {
  ...
  try {
    ...
    const aduanaTexto = req.query.aduanaTexto || req.query.aduanatexto || process.env.ADUANA_TEXTO || process.env.ADUANATEXTO;
    const anio = req.query.anio || process.env.ANIO;

    if (!url) return res.status(400).json({ error: 'Missing url' });
    if (!patente || !documento || !aduanaTexto || !anio) {
      return res.status(400).json({ error: 'Missing params: patente, documento, aduanaTexto, anio' });
    }

    const headless = (req.query.headless ?? process.env.HEADLESS ?? 'true') !== 'false';
    const waitUntil = req.query.waitUntil || process.env.WAIT_UNTIL || 'networkidle2';
    ...
    return res.json({ meta: { url, patente, documento, aduanaTexto, anio, count: data.length, ts: new Date().toISOString() }, data });
  } catch (err) {
